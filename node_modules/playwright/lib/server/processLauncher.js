...
class EventEmitterWrapper extends events_1.EventEmitter {
    constructor(emitter) {
        super();
        this.setMaxListeners(0);
        this._wrappedEvents = new Set();
        for (const method of ['addListener', 'on', 'once', 'prependListener', 'prependOnceListener']) {
            this[method] = (event, listener) => {
...
async function launchProcess(options) {
    ...
    const listeners = [helper_1.helper.addEventListener(processWrapper, 'exit', killProcess)];
    if (options.handleSIGINT) {
        listeners.push(helper_1.helper.addEventListener(processWrapper, 'SIGINT', () => {
    ...
    if (options.handleSIGTERM)
        listeners.push(helper_1.helper.addEventListener(processWrapper, 'SIGTERM', gracefullyClose));
    if (options.handleSIGHUP)
        listeners.push(helper_1.helper.addEventListener(processWrapper, 'SIGHUP', gracefullyClose));
